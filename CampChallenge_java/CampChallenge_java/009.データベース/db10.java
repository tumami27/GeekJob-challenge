    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try{
                ...
                db_con = DriverManager.getConnection("jdbc:mysql://localhost:3306/challeng_db","matumoto","aaaa");
               
                db_st =db_con.prepareStatement("update profile set neme=? tell=? age=? birthday=? where profilesID = ?" );
                db_st.setString(1,dName );
                db_st.setString(2, dTell); 
                db_st.setInt(3, Integer.parseInt(dAge));
                db_st.setString(4,dBirthday);
                db_st.setInt(5, Integer.parseInt(dID));
                db_st.executeUpdate();
                db_st.close();
                db_con.close();
